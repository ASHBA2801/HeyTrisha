 ...
 * a custom credentials table, not WordPress core tables. Direct $wpdb usage
 * is the standard approach for custom plugin tables.
 * 
 * phpcs:disable WordPress.DB.DirectDatabaseQuery -- Custom credentials table requires direct database access
 */
class HeyTrisha_Secure_Credentials {
    ...
    public function set_credential($key, $value) {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Table name ($table_name) uses $wpdb->prefix which is sanitized by WordPress core
        $existing = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM $table_name WHERE credential_key = %s", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    ...
    public function get_credential($key, $default = '') {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Table name ($table_name) uses $wpdb->prefix which is sanitized by WordPress core. Table names cannot be parameterized in SQL. $key is properly parameterized with %s placeholder.
        $encrypted_value = $wpdb->get_var($wpdb->prepare(
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name ($table_name) cannot be parameterized
    ...
    public static function verify_setup() {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Table existence check; $table_name uses $wpdb->prefix which is sanitized by WordPress core
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Counting credentials; $table_name uses $wpdb->prefix which is sanitized by WordPress core
